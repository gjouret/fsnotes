    ...
    public static func parse(_ markdown: String) -> Document {
        ...
        while i < lines.count {
            ...
            if !rawBuffer.isEmpty, let setextLevel = ATXHeadingReader.detectSetextUnderline(line),
               !ListReader.isEmphasisOnlyParagraph(rawBuffer) {
            ...
            if let result = ListReader.read(
                ...
                parseInlines: { parseInlines($0, refDefs: refDefs) },
                interruptsLazyContinuation: interruptsLazyContinuation,
                parseRecursive: { MarkdownParser.parse($0) },
                refDefs: refDefs
            ) {
                flushRawBuffer()
                blocks.append(.list(items: result.items, loose: result.loose))
                i = result.nextIndex
                continue
            }
            ...
            if rawBuffer.isEmpty,
               ListReader.leadingSpaceCount(line) >= 4 {
                var codeLines: [String] = []
                codeLines.append(ListReader.stripLeadingSpaces(line, count: 4))
                var jj = i + 1
                while jj < lines.count {
                    ...
                    if nextLine.isEmpty || isBlankLine(nextLine) {
                        ...
                        while kk < lines.count {
                            ...
                            if ListReader.leadingSpaceCount(peek) >= 4 {
                        ...
                        if foundContinuation {
                            ...
                            // not an empty line.
                            if ListReader.leadingSpaceCount(nextLine) >= 4 {
                                codeLines.append(ListReader.stripLeadingSpaces(nextLine, count: 4))
                            } else {
                                codeLines.append("")
                            }
                            jj += 1
