...
### Status snapshot (2026-04-28, updated post-12.C.6.j)
...
| 12.C.6.k — Empty-marker items owning indented code (spec #278) | ✅ SHIPPED | (this commit) | The empty-marker-content-fill branch in the inner-while-loop already detects `dedented` indent ≥ 4 as a "block starter" and skips the cur.content fill (cur.content can only hold paragraph-style text). The bug: it also fell out of the routing entirely, leaving `      baz` to terminate the list and become a top-level indented code block. New sibling branch added: when `last.content.isEmpty && last.continuationLines.isEmpty && lineIndent >= cc && dedentedIs4Indent`, walk forward collecting all subsequent lines indented ≥ cc into the item's `continuationLines` (with the same stop conditions as the existing block-starter-as-continuation branch). `buildItemTree`'s else branch then re-parses the dedented continuation as `parse("    baz\n") → [.codeBlock("baz")]`, and the renderer emits `<li>\n<pre><code>baz\n</code></pre>\n</li>` per spec. Test threshold bumped: List items 44 → 45. **List items: 44/48 → 45/48 (94%, closing #278)**. Overall **643/652 → 644/652 (98.8%)**. |
| 12.C.6 — Push CommonMark from 95.1% to 100% (residual failures) | 🚧 IN PROGRESS | (a–k shipped) | With combinator architecture in place, residuals become small grammar edits. **Slices 12.C.6.a–k shipped: 24 spec fixes against the 620 baseline.** Remaining 8 failures: List items + Lists (6, all multi-block items needing further work — #290, #292, #293, #312, #313, #325); Tabs (1, #9, tab-as-indent in nested list); Images (1, #590, accepted FSNotes++ wikilink-extension non-conformance). The structural split between `ListItem.children: [ListItem]` and `continuationBlocks: [Block]` blocks #325 (ordering issue: paragraph after sublist must render BETWEEN sublist and item-end, but current data model places continuation BEFORE children) — closing it requires the `ListItem.children: [Block]` redesign tracked as a candidate Phase 11/Slice B. |
...
Eleven slices have shipped (12.C.6.a–k, commits `2d05378 → (12.C.6.k)`), closing 24 spec examples (#218, #540, #541, #548, #559, #568, #238, #524, #526, #536, #538, #518, #519, #520, #532, #533, #175, #318, #320, #321, #324, #289, #300, #278) and lifting compliance from 620/652 (95.1%) to 644/652 (98.8%). Four buckets reached 100% (Links 90/90, Block quotes 25/25, Link reference definitions 27/27, HTML blocks 44/44). See the status-snapshot table at the top of Phase 12 for per-slice details and the matching ARCHITECTURE.md sections for prose write-ups.

The remaining 8 failures cluster into two structural buckets, both larger than the small grammar edits the .a–.k ladder has been delivering:

- **Multi-block list-item children (6 failures: #290, #292, #293, #312, #313, #325 + 1 family-adjacent failure #9 Tabs).** Phase 12.C.6.i closed five (#175, #318, #320, #321, #324) by routing indented block-starters into `continuationBlocks` and re-parsing combined first-line + continuation when the first line opens a non-paragraph block. Phase 12.C.6.j closed #289 (top-level 4-space-indent → indented code) and #300 (setext-underline-as-first-continuation in tight items). Phase 12.C.6.k closed #278 (empty-marker items owning indented code). The remaining failures need different fixes: 0-space-indented lazy continuation through item content column (#290), nested-blockquote lazy continuation through stripped prefixes (#292, #293), the inner-while-loop break-out rule for marker indent ≥ outer item content column (#312, #313), and the `[ListItem]`-vs-`[Block]` ordering issue for paragraph-after-sublist (#325). #325 specifically requires the `ListItem.children: [Block]` redesign so a sublist and a continuation paragraph can interleave correctly.
...
Per-bucket gating remains intact: every commit holds existing buckets at or above their pre-commit pass count, and the threshold floors in `CommonMarkSpecTests` were bumped to lock in each slice's gain (Block quotes 17 → 25, Link reference definitions 22 → 27, Links 75 → 85 → 90, HTML blocks 40 → 44, Lists 9 → 23, List items 12 → 44 → 45).
