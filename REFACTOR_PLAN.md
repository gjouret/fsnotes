...
**Tier C — delete the legacy `NotesTextProcessor.highlight*` markdown styler — ✅ SHIPPED 2026-04-25 (`75daca5`).** `NotesTextProcessor.swift` reduced from 1,618 → 423 LoC (1,195 deleted); `Tests/RendererComparisonTests.swift` deleted entirely (379 LoC); dead `quoteIndendation` / `quoteIndendationStyle` removed for free; `legacyMarkdownHighlight` rule-7 pattern dropped; comment cleanups in 5 files. Surviving `NotesTextProcessor` members all have external callers (`applySyntaxHiding`, `getHeaderFont`, color/font statics, `appUrlRegex`, `codeSpanRegex`, `MarklightRegex` struct). `Phase44SourceModeTests` 9/9 pass; `SystemIntegrityTests` 33/33 pass; `EditorHTMLParityTests` 85/85 pass; `InlineTagTests` 7/7 pass; rule-7 gate clean.

**Tier C follow-up — retire `InlineTagRegistry.processInlineTags` + `NotesTextProcessor.applySyntaxHiding` — ✅ SHIPPED 2026-04-28.** The Tier C ship entry flagged `processInlineTags` as test-only (zero production callers). Investigation confirmed both `processInlineTags` (~50 LoC including doc comment) and its sole helper `NotesTextProcessor.applySyntaxHiding` (~35 LoC) were live only via `Tests/InlineTagTests.swift`. `applySyntaxHiding`'s docstring named `TextStorageProcessor.hideSyntaxRange` as a second caller, but that symbol does not exist anywhere in the codebase — the docstring was stale. Production rendering of the three inline-HTML tags (`<u>`, `<kbd>`, `<mark>`) lives in `InlineRenderer.swift:225–254` (kbd) and `InlineRenderer.swift:193–204` (highlight, theme-driven), parallel implementations that do not consult the registry at runtime. `buildInlineTagDefinitions` survives because `SystemIntegrityTests.test_markHighlight_hasVisibleAlpha` reads the mark definition's alpha as a config-invariant reference. Three deletions: (1) `Tests/InlineTagTests.swift` (99 LoC), (2) `processInlineTags` from `FSNotesCore/Rendering/InlineTagRegistry.swift`, (3) `applySyntaxHiding` from `FSNotesCore/NotesTextProcessor.swift`. `project.pbxproj` updated in 4 places (PBXBuildFile, PBXFileReference, PBXGroup children, PBXSourcesBuildPhase). Build clean; rule-7 gate clean; `Phase44SourceModeTests` 9/9, `SystemIntegrityTests` 33/33 across all six classes (`SavePathConvergenceTests`, `FillIdempotencyTests`, `StateConsistencyTests`, `EditSerializeRoundTripTests`, `SpliceConsistencyTests`, `ProjectionCoverageTests`) green. The retirement closes the last marker-hiding-trick (`Color.clear` + per-character negative `.kern`) call site in non-whitelisted production code; the technique remains banned by Invariant D and the rule-7 gate's `clearForeground` / `negativeKern` patterns.

**Tier C — original PLANNED entry below preserved for history:** Phase 4.4 (`6b875ab`) flipped production source-mode rendering to `SourceRenderer` and retired the 6 production call sites of `NotesTextProcessor.highlight*`. The function bodies were retained as the A/B-comparison witness for `Tests/RendererComparisonTests.swift` and one test in `Tests/SystemIntegrityTests.swift`. The rule-7 `legacyMarkdownHighlight` grep gate prevents new production callers but does not delete the functions themselves. As of 2026-04-25 the A/B comparator's value has decayed: `SourceRenderer` has been the canonical source-mode path for ~2 weeks across all production code paths with zero regressions traced to it.
...
**Follow-up: live regression — wire-up fix shipped `f219de1` (2026-04-24); chicken-and-egg for mermaid/math remains.** Initial user report: the `</>` hover button doesn't appear on code blocks or mermaid in the live app. The lazy-associated-object hypothesis (no production reader → overlay never constructed) was the wrong cause: `EditTextView+BlockModel.swift:491` reads `vc.codeBlockEditToggleOverlay.reposition()` alongside `vc.tableHandleOverlay.reposition()` inside the deferred Phase-2 async block of `fillViaBlockModel(note:)`. Commit `f219de1` ("Fix table-handle + code-block-toggle overlay mount timing") landed both reposition calls together. **If the bug still reproduces post-`f219de1`**, the residual cause is a different class: `CodeBlockEditToggleOverlay.swift:295` filters fragments via `guard fragment is CodeBlockLayoutFragment else { return true }` — it does NOT match `MermaidLayoutFragment` or any math fragment. Slice 1's design downgrades a mermaid/math block to `.codeBlock` rendering ONLY when the block is already in the editing set, so the toggle button can never appear on a mermaid block to OPEN it (chicken-and-egg). Two viable fixes: (a) broaden the overlay filter to also match Mermaid/Math layout fragments, or (b) re-render the toggle on top of the rendered fragment regardless of class. Tracked as a Phase 8 follow-up slice, not yet scheduled — needs user confirmation that the regression is still live before dispatching.
...
29. Click in top-left cell paints caret ABOVE the cell — ⚠️ **REOPENED 2026-04-25** after user reports the symptom still happens in the live app. Original fix `de68ca6` (545 LoC across `EditTextView+Appearance.swift`, `TableElement.swift`, `TableLayoutFragment.swift`, `TableCaretRectTests.swift`) overrode caret painting + added cell-rect helpers, with all 303 added tests + the existing TableNav/TableCellEditing/TableCellHitTest suites green. So either: (a) the fix has a code-path gap (e.g. covers populated cells but not empty ones, or one click region but not the column-handle-strip area where the cursor still lands), or (b) a later commit regressed it. Investigation: take a fresh diagnostic dump from the live app (cell click → log proposed range from `selectedRange(forProposedRange:)` + actual caret rect that gets drawn vs `TableLayoutFragment`'s computed cell rect for that storage offset). The `de68ca6` test suite is green, so whichever cell shape / click path the user is exercising isn't covered by the existing tests — add a regression test for THAT path before fixing.
30. Tab inside a table cell inserts a literal `\t` ✅ (just shipped: `f55f65a`)
31. Code block with a blank line in the middle — the background shading is interrupted (white) at the blank line and resumes on the next code line. Shading should span the full code block including blank lines. ✅ Fixed: removed the `frame.width > 0` early-return guard in `CodeBlockLayoutFragment.drawBackground` — empty paragraphs (the blank line) had `layoutFragmentFrame.width == 0`, suppressing the bg paint while their `frame.height` (= line height + paragraphSpacing) was non-zero. The container-derived `width > 0` guard further down is the correct degenerate-case backstop. Regression test: `test_bug31_codeBlockFragment_blankLineInMiddle_continuousShading` (composes the first + middle fragments into a magenta-fill bitmap and asserts both center pixels share the code-block bg color).
32. Shift-Tab from (0,0) of a table wraps to the LAST cell instead of staying at (0,0). Modular arithmetic in `moveToAdjacentCell` should not wrap. ✅ (`d2e8294`: clamp `targetIdx` to `[0, totalCells-1]` instead of `((idx + delta) % totalCells + totalCells) % totalCells`. Shift-Tab at (0,0) and Tab at last-cell now both stay put. New `pressTab` / `pressShiftTab` verbs added to `EditorScenario`. 2 new bug-edge tests + renamed `test_T2d_tabKey_wrapsToNextRow` → `test_T2d_tabKey_advancesAcrossRowBoundary` since it tested an internal row crossing, not the wrap edge.)
...
34. `insertTableMenu` source-mode path inserts a single `\n` before the table instead of a blank line (`\n\n`). Result is invalid GFM when the table follows a paragraph: the table's first row gets eaten as paragraph continuation by external GFM parsers (GitHub, Obsidian, Bear). Block-model path is fine — `MarkdownSerializer` already joins blocks with `\n\n` on save. ✅ (`04c0c7d`: extracted `EditTextView.tablePrefixForSourceModeInsertion(at:in:)` pure helper. Returns `""` at doc-start, `"\n"` after an existing `\n`, `"\n\n"` otherwise — guarantees a blank line precedes the table. `test_insertTable_sourceMode_prefixHelper_contract` covers all four cases; `test_insertTable_sourceMode_endToEnd_prefixIsBlankLine` exercises the full IBAction)
35. Gutter glyphs (H1/H2/triangle) are not vertically center-justified on their headers. H1's glyph is bottom-aligned; H2-H6 glyphs are top-aligned. Should be vertically centered on the heading's text baseline / cap-height region. Lives in `GutterController.swift` glyph layout. ✅ (`406357f`: `GutterController.visibleHeadingsTK2` now centers on `fragment.textLineFragments.first.typographicBounds.midY` instead of the fragment frame's geometric midY. Per-heading-level `paragraphSpacingBefore`/`After` biases were pushing the fragment midY off the actual glyph row.)
...
48. Pasting a table copied from an external spreadsheet app (Numbers, Excel, Google Sheets, etc.) does not produce a markdown table in WYSIWYG mode. **The same paste in source / Markdown mode works** — i.e. source-mode's paste handler is converting (probably from `public.html`'s `<table>` or from TSV `public.utf8-plain-text`) into real GFM markdown, which then renders correctly when the note is read. WYSIWYG-mode paste either skips the conversion or treats the pasteboard text as a single paragraph (cell separators end up as soft breaks / literal tabs). User-reported 2026-04-25. **Investigation order:** (1) find what the source-mode paste handler does that WYSIWYG paste doesn't — diff the two dispatchers. The conversion logic almost certainly already exists; the WYSIWYG path just isn't routing through it. (2) Likely fix: in `EditTextView+Pasting.swift` or whichever path the WYSIWYG paste dispatcher uses, run the existing TSV/HTML→GFM conversion BEFORE `EditingOps.pasteIntoParagraph` / `replaceBlockRange`. The output is markdown text; from there the existing paste-into-block infrastructure handles the rest.
49. (Removed — duplicate of #29; the symptom the user re-reported on 2026-04-25 — cursor appearing above the table where the column handle is — is exactly the original #29 description. #29 has been re-opened from ✅ to ⚠️ above.)
50. Settings → Editor tab is missing 5 checkboxes that the original FSNotes had: Focus in editor when selecting note, Automatically insert closing braces and close quotes, Scan inline tags, Clickable links, Code block live highlighting. User-reported with side-by-side screenshots 2026-04-25. Root cause: commit `7c74195` ("Remove Images Width preference") deleted the Images Width slider's `<slider id="Vrr-sG-fOD">` and its label `dmd-5j-kY7` view elements from `FSNotes/Base.lproj/Main.storyboard` but left 9 constraints orphaned referencing those phantom IDs. ✅ (`9b1be2b`: 9 phantom constraints deleted; new constraints `B50-MS-top0` (Margin Size top → Line Width bottom + 8) and `B50-MS-tra0` (Margin Size trailing → Line Width trailing) added; deployed app launches cleanly with no constraint warnings — user must visually confirm the 5 checkboxes now render below Margin Size.)
51. Code spans (inline `` `code` ``) render in monospaced font but **without the subtle rounded-rect light-gray chrome** that the original FSNotes drew around them. User-reported with screenshot 2026-04-25. Diagnosis: `theme.chrome.inlineCodeBackground` was defined in `ThemeSchema.swift:297` but never read anywhere; the `.code` inline case in `InlineRenderer.swift` only applied the monospaced font. ✅ (`0a32694` / `82bc196` / `6579cda`: Option B — full fidelity. New `.inlineCodeRange` attribute key tags code-span runs at `InlineRenderer`. New `ParagraphLayoutFragment` + `InlineCodeChromeDrawer` paint pass (~145 LoC) traces tagged ranges and paints rounded-rect chrome BEFORE the default text draw. Theme gains `inlineCodeBackground` / `inlineCodeBorder` / `inlineCodeBorderWidth` / `inlineCodeCornerRadius` slots in `ThemeChrome`; default + Dark themes ship fill-only, High Contrast adds 1pt border for accessibility. 13 regression tests in `Tests/InlineCodeChromeTests.swift`. ~270 LoC net.)
52. Headings don't parse inline markdown for code spans (or any inline syntax). User-implicit, surfaced 2026-04-25 by the bug #51 agent. `HeadingRenderer` flattens its `suffix` to a single `[.text(suffix)]` so backticks in `# Heading with ``code``` render as literal text — and by extension `**bold**`, `*italic*`, `[link](url)` etc. inside a heading body don't render either. Confirmed limitation in the inline-code chrome test file. **Fix:** call `MarkdownParser.parseInlines(suffix, refDefs:)` on the heading suffix before passing to `InlineRenderer.render`, mirroring how paragraphs and list items already do this. Likely 5-10 LoC in `HeadingRenderer.swift`. Out of scope for #51's chrome work; tracked separately.
54. After unfolding a folded header, list bullets (and todo checkboxes) below it don't render until the user scrolls the list off-screen and back. Same redraw-after-state-change class as bug #19 (Numbers QuickLook thumbnail). User-reported 2026-04-26. Diagnosis: `TextStorageProcessor.toggleFold`'s unfold path re-splices `projection.attributed` over the fold range to restore foreground color, but `NSTextAttachmentViewProvider`s (`BulletAttachmentViewProvider`, `CheckboxAttachmentViewProvider`) for the attachment runs in that range aren't re-instantiated until TK2 re-runs its layout pass — which only happens on scroll. Fix: after the unfold splice, call `textLayoutManager.invalidateLayout(for:)` on the fold range to force a layout pass, OR force-reload attachment view providers explicitly (the pattern bug #19's `viewDidMoveToWindow` + `QuickLookThumbnailCache` used).
55. The folded-header `[...]` chip indicator doesn't paint until the user scrolls the header off-screen and back. User-reported 2026-04-26 alongside #54. Same redraw-after-state-change class. The chip is drawn by `HeadingLayoutFragment.drawFoldedIndicator` which checks the `.foldedContent` attribute on the char immediately following the heading element range. When `toggleFold` ADDS that attribute (line 422), no layout invalidation is fired for the heading's fragment, so its `draw(at:in:)` doesn't re-run until the next layout pass triggered by scroll. Same fix shape as #54: `textLayoutManager.invalidateLayout(for: headingRange)` after the attribute change.
53. AI toolbar button (`bubble.left.and.text.bubble.right`) doesn't open the chat panel. User-reported 2026-04-25. The wiring on paper looks correct: `FormattingToolbar.swift:119` adds the button with `action: #selector(ViewController.toggleAIChat(_:))`, `target: nil` (routes through first responder chain, standard pattern). `toggleAIChat:` is `@objc public` on `ViewController`. **Investigation needed:** the user is testing a deploy from ~14:30 which predates today's AI Phase 4 work. Either the deployed build never had the button wired live (it shipped before live verification), the responder chain doesn't actually reach ViewController from the toolbar's parent (different view hierarchy than expected), or `toggleAIChat`'s `editAreaScroll.superview` guard is bailing. Diagnostic: add a one-line log at the top of `toggleAIChat` + at the point the button is added to the toolbar; redeploy; click button; check whether the function is entered at all. If not entered, the responder chain is the cause.
...
### Status snapshot (2026-04-28, updated post-12.C.5.g)
...
| 12.C.5.c — HTML block reader port | ✅ SHIPPED | `e9b98e6` | `HtmlBlockReader.swift` (~340 LoC). Public `detect(_:)` for the 7-type discriminator, `read(lines:from:trailingNewline:rawBufferEmpty:)` walks the per-type close conditions, `endsOnLine(_:type:)` covers same-line closers for types 1–5. 2 cross-cutting callers rewired (block loop + `interruptsLazyContinuation` lazy-cont check). MarkdownParser shrinks by ~270 LoC (38-line block-loop branch collapsed to a 9-line read; 5 private helpers — `detectHTMLBlock`, `htmlBlockEndsOnLine`, `htmlBlockTags`, `extractHTMLTagName`, `isCompleteHTMLTag` — moved to the reader). 30 new tests. "HTML blocks" 43/44 (98%) held. |
| 12.C.5.d — Blockquote reader port | ✅ SHIPPED | `661d973` | `BlockquoteReader.swift` (~210 LoC). Public `detect(_:)` returns the verbatim `(prefix, content)` pair (covers spec #6 partial-tab consumption). `innerAllowsLazyContinuation(_:)` exposes the FencedCodeBlockReader-aware paragraph-context test. `read(lines:from:trailingNewline:parseInlines:interruptsLazyContinuation:)` walks the explicit-marker + lazy-continuation merge — `parseInlines` and `interruptsLazyContinuation` injected as closures because both depend on parser state (refDef table, list-marker rules) the reader doesn't own. 4 cross-cutting callers rewired (block loop + 2 use-sites of `detectBlockquoteLine` + `interruptsLazyContinuation`). MarkdownParser shrinks by ~120 LoC (37-line block-loop branch collapsed to 11 lines; 2 private helpers — `detectBlockquoteLine`, `blockquoteInnerAllowsLazyContinuation` — moved to the reader). 28 new tests. "Block quotes" 24/25 (96%) held. |
| 12.C.5.e — Table reader port | ✅ SHIPPED | `858e0df` | `TableReader.swift` (~190 LoC). Public `read(lines:at:rawBuffer:trailingNewline:parseInlines:)` covers both detection modes (a: header on current line + separator next; b: header buffered as paragraph, separator on current line) and emits `(block, nextIndex, headerFromBuffer)` — the caller still owns `rawBuffer.removeLast()` for mode (b) before flushing. Public `isRow`, `isSeparator`, `parseRow`, `parseAlignments` for testability. `parseInlines` injected as a closure (refDefs intentionally empty inside cells, matching legacy behaviour). Single cross-cutting caller rewired (block loop). MarkdownParser shrinks by ~158 LoC (`detectTable` + `TableDetection` struct + `isTableRow` + `isTableSeparator` + `parseAlignments` + `parseTableRow` — 5 private helpers and a private struct moved to the reader; the 13-line block-loop branch collapsed to 17 lines via the new closure-passing call). 27 new tests in `TableReaderTests`. No CommonMark spec bucket for tables (GFM extension); HTML blocks 43/44 + Block quotes 24/25 buckets held; full corpus + parser + editing primitives all green. |
| 12.C.5.f — List reader port (per-line classifier only) | ✅ SHIPPED | (this commit) | `ListReader.swift` (~225 LoC). Conservative scope: only the per-line classifier surface ports — `ListReader.ParsedListLine` (now `public struct`), `parseListLine`, `listMarkerType`, `isOrderedListMarkerWithNonOneStart`. The ~320-line block-loop multi-line collection code, `buildItemTree` (recursive item-tree builder, calls back into `MarkdownParser.parse`), `deepestOwner`, `leadingSpaceCount`, and `stripLeadingSpaces` STAY in `MarkdownParser` — they weave through container-block continuation rules and recursive parser entry, and porting them cleanly is its own slice (potentially `12.C.5.g`). Cross-cutting callers rewired via mechanical qualifier substitution (15+ call sites in `MarkdownParser.parse`, `buildItemTree`, `deepestOwner`, `interruptsLazyContinuation` — all qualified with `ListReader.X`). MarkdownParser shrinks by ~225 LoC. 30 new tests in `ListReaderTests` covering bullet markers, ordered markers, indent, empty items, marker-without-space rejection, HR-disambiguation rejection, the spec #7 indented-code-after-tabby-marker case, checkbox extension, ordered-with-bracketed-content (must NOT detect checkbox), `listMarkerType` family classification, `isOrderedListMarkerWithNonOneStart`, and end-to-end MarkdownParser.parse round-trips for bullet / ordered / todo / family-change / non-1 ordered paragraph-non-interrupt. List items 42/48 (88%) + Lists 19/26 (73%) buckets held. |
| 12.C.5.g — List multi-line collection port | ✅ SHIPPED 2026-04-28 | (this slice) | `ListReader.read(lines:from:rawBuffer:trailingNewline:parseInlines:interruptsLazyContinuation:parseRecursive:refDefs:) -> ReadResult?` ports the ~556-line block-loop list-collection-and-build section out of `MarkdownParser.parse`. The full helper kit moves with it: `buildItemTree` (recursive item-tree builder, now public), `deepestOwner`, `canAppendListMarker`, `leadingSpaceCount`, `stripLeadingSpaces`, `isEmphasisOnlyParagraph`. Closure surface kept tight — four arguments: `parseInlines`, `interruptsLazyContinuation`, `parseRecursive` (callback to `MarkdownParser.parse`), `refDefs` (passed through). Direct calls to other readers (`ATXHeadingReader.detect`, `FencedCodeBlockReader.detectOpen`, `HorizontalRuleReader.detect`, `BlockquoteReader.detect`, `HtmlBlockReader.detect`, `ATXHeadingReader.detectSetextUnderline`) stay direct because all readers live in `Combinators/`. The trivial `isBlankLine` helper is duplicated locally as a private 3-line `isBlank` to avoid plumbing yet another closure. Two cross-cutting call-site updates outside the list block: the outer-loop setext detection at line 100 (`!ListReader.isEmphasisOnlyParagraph(rawBuffer)`) and the indented-code-block branch (~7 call sites for `ListReader.leadingSpaceCount` + `ListReader.stripLeadingSpaces`). MarkdownParser shrinks by ~888 LoC (2342 → 1454); ListReader grows by ~975 LoC (277 → 1252). Net +87 LoC across the two files (closure plumbing + doc comments). All list-related test suites green: `ListReaderTests` + `BlockParserTests` + `ListEditingFSMTests` 113/113; `MarkdownParserTests` + `MarkdownSerializerTests` + `EditorHTMLParityTests` + `ListRoundTripTests` 112/112; `CommonMarkSpecTests` 36/36 with overall compliance 651/652 (99.8%) intact. Rule-7 gate clean. Closes the Phase 12.C parser-decomposition arc — every reader (HardLineBreakParser, CodeSpanParser, MathParser, StrikethroughParser, WikilinkParser, EntityParser, AutolinkParser, RawHTMLParser, LinkParser, EmphasisResolver, LinkResolver, FencedCodeBlockReader, HorizontalRuleReader, ATXHeadingReader, HtmlBlockReader, BlockquoteReader, TableReader, ListReader) now lives in its own combinator file with explicit closure dependencies. |
| 12.C.6.a — Nested-blockquote ref-def discovery (spec #218) | ✅ SHIPPED | `2d05378` | `collectLinkRefDefs` now strips blockquote prefixes per line into a parallel `strippedLines` view, attempts a multi-line ref-def parse there when the leading line carries a `>` prefix and the parse is well-formed, registers the ref-def in `refDefs`, and returns the source-line indices via a new third return value `blockquoteRefDefLines: Set<Int>`. The block-parse loop unpacks the third return value and threads it as `skipLines` to `BlockquoteReader.read(...)`, which walks past those lines without emitting `BlockquoteLine`s — preserving the (now empty) blockquote container while hoisting the ref-def to document scope. `BlockquoteReader.read(...)` gains a `skipLines: Set<Int> = []` parameter (default-empty for back-compat with all other callers). 2 new tests in `BlockquoteReaderTests` (ref-def-only blockquote → empty container; mixed quote with hoisted ref-def keeps other lines). **Link reference definitions: 26/27 → 27/27 (100%)**. Block quotes 24/25, HTML blocks 43/44 unchanged. Overall CommonMark 620/652 → **621/652 (95.2%)**. |
| 12.C.6.b — Unicode case fold for ref label normalization (spec #540) | ✅ SHIPPED | `495c121` | `normalizeLabel` switched from `.lowercased()` to `.folding(options: .caseInsensitive, locale: nil)`. CommonMark §4.7 specifies Unicode case folding (which handles multi-codepoint folds like ẞ → "ss"); `lowercased()` only does single-codepoint case mapping (ẞ → ß), so registration and lookup of labels containing such characters fell out of agreement. Symmetric change — both registration and lookup pass through the same helper. **Links: 76/90 → 77/90 (85%)**. Overall **622/652 (95.4%)**. |
| 12.C.6.c — Newline-aware ref label normalization (spec #541) | ✅ SHIPPED | `ecbba98` | `normalizeLabel` now collapses every kind of whitespace run uniformly via `components(separatedBy: .whitespacesAndNewlines).filter { !$0.isEmpty }.joined(separator: " ")`. Multi-line ref-def labels (e.g. `[Foo\n  bar]`) used to end up with a literal `\n` in the normalized form because the previous implementation only split on " " and "\t". **Links: 77/90 → 78/90 (87%)**. Overall **623/652 (95.6%)**. |
| 12.C.6.d — Wikilink yields to brackets and ref-defs (specs #548, #559) | ✅ SHIPPED | `7d6fc44` | Two narrow rules. (i) **Bracket-integrity** in `WikilinkParser.match`: an extra `[` immediately before `[[` or an extra `]` immediately after `]]` rejects the wikilink (spec #548 `[[[foo]]]` stays plain text). (ii) **Ref-def precedence** at the call site in `MarkdownParser.tokenizeNonEmphasis`: when the wikilink target normalizes to a known ref-def label, the standard reference-link path wins — the outer `[` is literal, the inner `[target]` resolves through the ref-def, the closing `]` is literal (spec #559). The wikilink-as-fallback semantics for unmatched targets are preserved (spec #590 stays the documented FSNotes++ extension). 2 new tests in `WikilinkParserTests` for the bracket-integrity rule and one for the ref-def-precedence rule. **Links: 78/90 → 80/90 (89%)**. Overall **625/652 (95.9%)**. |
| 12.C.6.e — Shortcut ref-link works after failed inline link (spec #568) | ✅ SHIPPED | `a2b4ef7` | `tryMatchReferenceLink` previously rejected the shortcut path whenever the closing `]` was followed by `(`, on the theory that `(` always indicates an inline link. That's too eager: the function is only consulted after `LinkParser.match` has already failed, so reaching it guarantees the `(...)` is NOT a valid inline-link destination. Removing the guard lets the shortcut activate as the spec intends — `[foo](not a link)` with `[foo]: /url1` defined now renders as ref-link `<a>foo</a>` followed by literal `(not a link)`. **Links: 80/90 → 81/90 (90%)**. Overall **626/652 (96.0%)**. |
| 12.C.6.f — 4-space-indented marker can't interrupt a paragraph (spec #238) | ✅ SHIPPED | `6264bc5` | Added the third "don't interrupt a paragraph" rule to the block-parse loop's list-detection branch. CommonMark §4.4 / §5.2: a list marker indented 4+ spaces is lazy continuation, not a new block. The loop already had bare-marker-at-EOL and ordered-start≠1 guards; `!(rawBuffer.count > 0 && leadingSpaceCount(firstParsed.indent) >= 4)` is the third. Spec #238 `> foo\n    - bar` now keeps `- bar` as paragraph continuation inside the blockquote. **Block quotes: 24/25 → 25/25 (100%)**. Overall **627/652 (96.2%)**. |
| 12.C.6.g — Link bracket scan respects HTML / autolink boundaries (specs #524, #526, #536, #538) | ✅ SHIPPED | `07a55c9` | `LinkParser.match` (inline) and `tryMatchReferenceLink` (ref) both scan the link text for the matching `]` via bracket-depth tracking. Brackets inside an HTML attribute value or an autolink URL must NOT count — they're inside another inline construct. When the scan sees `<`, it now tries `AutolinkParser.match` then `RawHTMLParser.match`; a success advances the cursor to the construct's end and the bracket count is unaffected. **Links: 81/90 → 85/90 (94%, fixing #524, #526, #536, #538)**. Overall **631/652 (96.8%)**. |
| 12.C.6.h — Link-in-link literalization via §6.4 delimiter stack (specs #518, #519, #520, #532, #533) | ✅ SHIPPED | `14e38f6` | New `Combinators/LinkResolver.swift` (~250 LoC) carries the §6.4 delimiter-stack algorithm: a pre-pass over the inline character stream that pushes `[` / `![` openers, walks each `]` against the most recent active opener, and — on successful resolution — marks earlier `[` openers (NOT `![`) as inactive. The greedy left-to-right link/image matching previously embedded in `MarkdownParser.tokenizeNonEmphasis` is replaced with a `linksByOpenIdx` lookup populated from the resolver's output. `LinkParser.parseInlineBody` factored out as a public helper so the standalone `LinkParser.match` and the resolver share identical body-parse semantics; `tryMatchReferenceLink` (~70 LoC) retired — its three forms (full / collapsed / shortcut) plus the spec #568 fall-through now live in `LinkResolver.matchAfterCloser`. 16 new tests in `LinkResolverTests`. **Links: 85/90 → 90/90 (100%, closing #518, #519, #520, #532, #533)**. Overall **636/652 (97.5%)**. Two more buckets reach 100%: Links 90/90, and Block quotes 25/25 + Link reference definitions 27/27 already at 100% from earlier 12.C.6 slices. |
| 12.C.6.i — Multi-block list items via continuationLines + tight-mode rendering (specs #175, #318, #320, #321, #324) | ✅ SHIPPED | `6e93688` | Three coordinated changes close five list-multi-block failures without redesigning `ListItem`. (1) `MarkdownParser.parse` list-block branch: a new sibling to the lazy-continuation branch routes indented block-starters at the current item's content column or deeper into the item's `continuationLines` (mirroring the blank-line-then-content path without the blank prefix). Spec #320 `* a\n  > b\n  >` now collects the blockquote into *a's continuation. (2) `buildItemTree` re-parses `cur.content + continuationLines` as a single block sequence whenever the item's first line opens a non-paragraph block (fence, blockquote, ATX heading, HR, HTML block) — fixes the lazy-merge contamination where spec #318 `- ```\n  b\n\n\n  ```` merged "b" into the item's inline content as text instead of treating "```" as the fence open and "b\n\n\n" as the body. The blank-line filter on continuation blocks was lifted so the renderer's blank-line looseness signal can fire correctly. (3) `CommonMarkHTMLRenderer`: dropped the `count >= 2 → loose` heuristic in `itemHasLoosenessSignal` (CommonMark §5.4 keys looseness on blank-line-between-blocks, not block count), and tight-mode rendering now inserts a `\n` between inline content and the first continuation block so output reads `<li>a\n<blockquote>...` per spec. The list-level `hasBlankLines` flag is no longer set unconditionally when continuation collection consumes blanks — only when trailing blanks are trimmed (the actual "next item follows blank" signal). Test thresholds bumped: HTML blocks 40 → 44, Lists 9 → 23. **HTML blocks: 43/44 → 44/44 (100%, closing #175)**. **Lists: 19/26 → 23/26 (88%, closing #318, #320, #321, #324)**. Overall **636/652 → 641/652 (98.3%)**. |
| 12.C.6.j — Top-level 4-space-indented marker → indented code + setext-underline-as-first-continuation (specs #289, #300) | ✅ SHIPPED | `575431b` | Two narrow fixes against independent failure modes. (1) **Top-level list detection** (`MarkdownParser.parse` line ~155): the previous "don't open a list when paragraph-continuing with 4+ space indent" guard (12.C.6.f) was tightened to "don't open a list when 4+ space indent at top level, period" — CommonMark §5.2 rule 1 caps preceding indent at K∈[0,3]. With the guard relaxed, `    1.  A paragraph` (spec #289) falls through to the indented-code-block detector instead of opening a list. **List items: 43/48 (90%, closing #289)**. (2) **Setext continuation in `buildItemTree`** (`MarkdownParser.swift` line ~1210 + `CommonMarkHTMLRenderer.renderListItem` line ~277): when a list-item's first continuation line is a setext underline (`---`/`===`) and the item's content is a non-emphasis paragraph, route the item through the combined-reparse branch so the inner parse sees `Bar\n---\nbaz` as one input and emits `[setext h2 Bar, paragraph baz]` rather than `[HR, paragraph baz]`. The renderer's `renderBlock(_, inTightList:)` now also unwraps `<p>` tags for tight-list paragraphs (the path was already passed through but `inTightList: false` was hardcoded for continuation blocks). **List items: 43/48 → 44/48 (92%, closing #300)**. Test threshold bumped: List items 12 → 44. Overall **641/652 → 643/652 (98.6%)**. |
| 12.C.6.k — Empty-marker items owning indented code (spec #278) | ✅ SHIPPED | `6904e2b` | The empty-marker-content-fill branch in the inner-while-loop already detects `dedented` indent ≥ 4 as a "block starter" and skips the cur.content fill (cur.content can only hold paragraph-style text). The bug: it also fell out of the routing entirely, leaving `      baz` to terminate the list and become a top-level indented code block. New sibling branch added: when `last.content.isEmpty && last.continuationLines.isEmpty && lineIndent >= cc && dedentedIs4Indent`, walk forward collecting all subsequent lines indented ≥ cc into the item's `continuationLines` (with the same stop conditions as the existing block-starter-as-continuation branch). `buildItemTree`'s else branch then re-parses the dedented continuation as `parse("    baz\n") → [.codeBlock("baz")]`, and the renderer emits `<li>\n<pre><code>baz\n</code></pre>\n</li>` per spec. Test threshold bumped: List items 44 → 45. **List items: 44/48 → 45/48 (94%, closing #278)**. Overall **643/652 → 644/652 (98.8%)**. |
| 12.C.6.l — Tab expansion in list-item indent + afterMarker (spec #9) | ✅ SHIPPED | `957b9be` | `ListReader.parseListLine` returned `indent` and `afterMarker` as raw character substrings, which caused the ~20 downstream sites that use `.indent.count` and `.afterMarker.count` for list-nesting arithmetic to count tabs as 1 character instead of 1–4 virtual columns. Spec #9 input ` - foo\n   - bar\n\t - baz\n` measured baz's indent as 2 characters (tab+space); buildItemTree then compared 2 < bar's content column 5 and treated baz as a sibling at the wrong level. Fix: normalize both `indent` and `afterMarker` to virtual-column-equivalent expanded spaces inside `parseListLine` (new `expandTabsToSpaces(_:startingAt:)` helper applies CommonMark §2.2 4-stop tabstop semantics, with the indent run starting at column 0 and the afterMarker run starting at the marker column). After normalization `.count` everywhere automatically equals virtual-column count, so no caller needs to change. The simplification also subsumes the open-coded "spaces + tabs*4" tab math in `MarkdownParser.interruptsLazyContinuation` (which over-counted leading-position tabs as 4 cols regardless of column). 3 new tests in `ListReaderTests` cover the per-line classifier (`test_parse_tabIndent_expandsToVirtualCols`, `test_parse_tabAfterMarker_underFiveCols_expanded`) and the end-to-end nesting (`test_parse_endToEnd_tabNestedList`). Test threshold bumped: Tabs 2 → 11. **Tabs: 10/11 → 11/11 (100%)**. Overall **644/652 → 645/652 (98.9%)**. |
| 12.C.6.m — Invalid-marker rejection (specs #312, #313) | ✅ SHIPPED | `3b7d1f9` | The inner list-collection loop's `parseListLine` success path appended every parsed marker to `parsedLines` unconditionally, ignoring whether the marker indent was actually valid for the current open list. CommonMark §5.2 requires a marker to satisfy one of two indent conditions: marker indent ≤ topListEdge + 3 (sibling at the outermost level), OR marker indent ≥ the most recently appended item's content column (nested under the deepest open item). New helper `canAppendListMarker(_ parsed, parsedLines, topIndent)` encodes this; it gates the marker append at TWO sites — the BLANK-line peek branch (terminate the list when the next-non-blank marker is invalid; spec #313 `1. a\n\n  2. b\n\n    3. c\n` → list ends after b, `3. c` becomes top-level indented code) and the NON-BLANK branch (fall through to the existing lazy-continuation logic when the marker is invalid; spec #312 `- a\n - b\n  - c\n   - d\n    - e\n` → e merges into d's paragraph as `d\n- e`). The non-blank branch wraps `parseListLine(l)` via `flatMap` to convert "invalid marker" into the same control flow as "no marker", which keeps the else-branch lazy-continuation logic untouched. Test threshold bumped: Lists 23 → 25. **Lists: 23/26 → 25/26 (96%, closing #312, #313)**. Overall **645/652 → 647/652 (99.2%)**. |
| 12.C.6.n — `ListItem.body: [Block]` redesign (spec #325) | ✅ SHIPPED | `94c211b` | The pre-redesign data model split a list item's body into two segregated arrays — `children: [ListItem]` (sublists) and `continuationBlocks: [Block]` (paragraphs, code blocks, blockquotes, etc.) — and the renderer emitted continuation FIRST, then sublists LAST. CommonMark §5.2 example #325 (`* foo\n  * bar\n\n  baz\n`) needs the sublist to appear BETWEEN two paragraphs in the same item: `<li><p>foo</p><ul><li>bar</li></ul><p>baz</p></li>`. The flat fields can't express that ordering. **Redesign:** `ListItem.body: [Block]` becomes the single ordered storage; sublists appear as `Block.list(...)` entries interleaved with continuation blocks and `.blankLine` markers (the latter retained as the renderer's looseness signal). Legacy reads `children` and `continuationBlocks` survive as filtered views derived from `body`; the legacy `ListItem(... children:, continuationBlocks:)` initializer survives as a convenience that concatenates `continuationBlocks` first then `.list(items: children)` last (matching the pre-redesign join). All ~150 existing call sites and test fixtures compile unchanged via the legacy paths. **Parser change:** `ParsedListLine` gains `continuationLinesPost: [String]`; the inner-while-loop's deepestOwner-branch now routes continuation collection into `continuationLinesPost` whenever any child of the owner has already been appended to `parsedLines`. `MarkdownParser.buildItemTree` composes `body` as `[continuation_pre, .list(children, loose: false), continuation_post]`. **Renderer change:** `CommonMarkHTMLRenderer.renderListItem` walks `body` in source order, dispatching `.list` to `renderList` (sublist) and other blocks to `renderBlock(_, inTightList:)` so paragraph unwrapping in tight lists still works for continuation paragraphs. The looseness signal (`itemHasLoosenessSignal`) now reads `body.filter(non-list)` rather than the legacy `continuationBlocks`. **Serializer change:** `MarkdownSerializer.serializeItem` slices `body` into runs of contiguous non-list blocks vs. single sublists; non-list runs serialize as a Document fragment (re-indented to the item's content column) so embedded blank-line blocks round-trip naturally; sublist runs recurse via `serializeItem`. Test threshold bumped: Lists 25 → 26. New regression test `test_parse_paragraphAfterSublist_preservesOrder` in `ListRoundTripTests` pins the body order. **Lists: 25/26 → 26/26 (100%, closing #325)**. Overall **647/652 → 648/652 (99.4%)**. |
| 12.C.6.o — Multi-block-evidence lazy continuation (spec #290) | ✅ SHIPPED | `fa437c3` | The list-block branch's `validMarker == nil` path previously rejected lazy continuation when the incoming line sat at the list-item's marker indent or shallower (`lineIndent > last.indent.count` narrow rule). Strict CommonMark §5.1 lazy continuation merges any non-block-starter, non-blank line into an open paragraph, but going fully strict would re-merge the editor layer's `[list, paragraph]` Documents (which serialize without an explicit `.blankLine` separator as `- foo\nbar`) at load time. **Compromise:** also merge when a subsequent line proves the item has multi-block content. The new `multiBlockEvidence` look-ahead skips consecutive lazy-continuation candidate lines (non-blank, non-marker, non-block-starter), then any blank gap, and reports `true` when the next non-blank line is indented to ≥ the item's content column. Spec #290's `1.  A paragraph\nwith two lines.\n\n          indented code\n\n      > A block quote.` matches: the deeply-indented code line licenses lazy-continuing `with two lines.` into item 1's open paragraph. `buildItemTree`'s existing continuation re-parse then emits the indented code block and blockquote as `body` blocks of the same item, matching the spec's expected `<ol><li><p>...</p><pre><code>...</code></pre><blockquote>...</blockquote></li></ol>`. Editor-produced `- foo\nbar` (no deep follower) continues to parse as `[list, paragraph]` because the look-ahead finds no qualifying line — preserving the round-trip property the narrow rule was introduced for. Test threshold bumped: List items 45 → 46. **List items: 45/48 → 46/48 (96%, closing #290)**. Overall **648/652 → 649/652 (99.5%)**. |
| 12.C.6.p — Nested-blockquote lazy continuation via renderer prefix injection (specs #292, #293) | ✅ SHIPPED | `f998c6c` | `CommonMarkHTMLRenderer.renderBlockquote` reconstructs the blockquote's inner markdown from its `BlockquoteLine`s and re-parses to recurse into nested blocks. Spec #292 (`> 1. > Blockquote\ncontinued here.`) and #293 (`> 1. > Blockquote\n> continued here.`) need the second line — `continued here.` after the outer blockquote prefix is stripped — to lazy-continue the INNER blockquote's paragraph inside the list item, through three container levels (outer blockquote → ordered list item → inner blockquote). Our line-oriented parser doesn't model the multi-level container stack that strict CommonMark uses for cross-container lazy continuation, and the 12.C.6.o multi-block-evidence look-ahead doesn't activate here because there's no follower at ≥ contentCol. **Synthesise the equivalent at the renderer:** when a continuation line follows an inner-content line that opens a list-item-with-inner-blockquote chain, inject `(contentCol spaces) + "> "` before the line body. The new `lazyContainerPrefix(forLine:previousLine:)` helper detects the chain (`ListReader.parseListLine` on the previous line whose post-marker content opens with `BlockquoteReader.detect`) and emits the inheritance prefix; it skips injection when the current line would itself open a new block (heading, HR, fenced code, HTML block, list marker, blockquote marker) so legitimate top-level interrupts pass through unchanged. Apply to BOTH lazy-prefix and explicit-`> `-prefix lines: from the inner re-parse's perspective, both arrive as plain `continued here.`, so both need the same fix. After injection the inner markdown becomes `1. > Blockquote\n   > continued here.\n` for both specs; the list parser's existing block-starter-as-continuation branch collects the prefixed line into the item's `continuationLines`, and `buildItemTree`'s `firstLineIsBlockStarter` combined re-parse emits `[blockquote(["Blockquote", "continued here."])]` as the item's body — matching the spec's `<blockquote><ol><li><blockquote><p>Blockquote\ncontinued here.</p></blockquote></li></ol></blockquote>` shape. Test threshold bumped: List items 46 → 48. **List items: 46/48 → 48/48 (100%, closing #292, #293)**. Overall **649/652 → 651/652 (99.8%)**. |
| 12.C.6 — Push CommonMark from 95.1% to 100% (residual failures) | ✅ COMPLETE | (a–p shipped) | With combinator architecture in place, residuals became small grammar edits and one renderer-side fix. **Slices 12.C.6.a–p shipped: 31 spec fixes against the 620 baseline.** Seven buckets at 100% (Links 90/90, Block quotes 25/25, Link reference definitions 27/27, HTML blocks 44/44, Tabs 11/11, Lists 26/26, **List items 48/48**). Sole remaining failure: Images #590 — accepted FSNotes++ wikilink-extension non-conformance (`![[foo]]` resolves to a wiki link by product design, not literal text). Overall **651/652 (99.8%)**, the practical ceiling for FSNotes++ given the wikilink extension. |
...
- `FSNotesCore/Rendering/Combinators/` — `Parser.swift` (lib) + 18 ported parsers/resolvers/readers:
  ...
  - **Inline link/image resolution (12.C.6.h):** LinkResolver (the §6.4 delimiter-stack algorithm — owns link-in-link literalization, ref-def lookup, and the inactivation rule).
  - **Block readers (12.C.5):** FencedCodeBlockReader, HorizontalRuleReader, ATXHeadingReader, HtmlBlockReader, BlockquoteReader, TableReader, ListReader (per-line classifier in 12.C.5.f; multi-line collection + `buildItemTree` + helper kit in 12.C.5.g, 2026-04-28). Each reader owns its own slice of the parse pipeline; MarkdownParser.parse is now a thin block-loop dispatcher.
...
Sixteen slices have shipped (12.C.6.a–p, commits `2d05378 → f998c6c`), closing 31 spec examples (#218, #540, #541, #548, #559, #568, #238, #524, #526, #536, #538, #518, #519, #520, #532, #533, #175, #318, #320, #321, #324, #289, #300, #278, #9, #312, #313, #325, #290, #292, #293) and lifting compliance from 620/652 (95.1%) to 651/652 (99.8%). Seven buckets reached 100% (Links 90/90, Block quotes 25/25, Link reference definitions 27/27, HTML blocks 44/44, Tabs 11/11, Lists 26/26, **List items 48/48**). See the status-snapshot table at the top of Phase 12 for per-slice details and the matching ARCHITECTURE.md sections for prose write-ups.

**Sole remaining failure: documented FSNotes++ wikilink-extension non-conformance (#590).** `![[foo]]` with no matching ref-def resolves to a wikilink rather than literal text. Accepted, not fixed — wikilinks are a product feature and #590 is the boundary case where CommonMark and the extension disagree. 651/652 (99.8%) is the practical ceiling for FSNotes++ at this resolution; pure-CommonMark conformance would land at 652/652 only by removing the wikilink extension, which is out of scope for the refactor.

Phase 12.C.6 sub-slice ladder shipped:
- 12.C.6.i closed five list-multi-block failures (#175, #318, #320, #321, #324) by routing indented block-starters into `continuationBlocks` and re-parsing combined first-line + continuation when the first line opens a non-paragraph block.
- 12.C.6.j closed #289 (top-level 4-space-indent → indented code) and #300 (setext-underline-as-first-continuation in tight items).
- 12.C.6.k closed #278 (empty-marker items owning indented code).
- 12.C.6.l closed #9 (tab expansion in list-item indent).
- 12.C.6.m closed #312 / #313 (invalid-marker rejection — lazy continuation of invalid markers into the deepest open paragraph; list termination when an invalid-marker follows a blank).
- 12.C.6.n closed #325 (paragraph-after-sublist source-order preservation) by replacing `ListItem.children: [ListItem]` + `continuationBlocks: [Block]` with a unified `body: [Block]` that interleaves sublists and continuation in source order.
- 12.C.6.o closed #290 (multi-block-evidence lazy continuation) — relax the narrow lazy-continuation rule when a subsequent line is indented to ≥ contentCol, proving the item has multi-block content. Editor round-trip preserved because plain `[list, paragraph]` Documents serialize to `- foo\nbar` with no deep follower, so the look-ahead doesn't activate on load.
- 12.C.6.p closed #292 / #293 (nested-blockquote lazy continuation through stripped prefixes) at the renderer side: `CommonMarkHTMLRenderer.renderBlockquote` now injects a `(contentCol spaces) + "> "` prefix on continuation lines whose previous content opens a list-with-inner-blockquote chain, so the inner re-parse routes the line into the inner blockquote via the existing block-starter-as-continuation branch.

Per-bucket gating remains intact: every commit holds existing buckets at or above their pre-commit pass count, and the threshold floors in `CommonMarkSpecTests` were bumped to lock in each slice's gain (Block quotes 17 → 25, Link reference definitions 22 → 27, Links 75 → 85 → 90, HTML blocks 40 → 44, Lists 9 → 23 → 25 → 26, List items 12 → 44 → 45 → 46 → 48, Tabs 2 → 11).
