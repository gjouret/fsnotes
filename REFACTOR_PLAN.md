...
- **T2-g — Hover + context menus + drag-reorder.** Split into sub-slices:
  ...
  - **T2-g.3/T2-g.4 Column drag-resize + width persistence — DEFERRED.** Handle strip renders and hit-tests; `TableHandleView.mouseDown` is a no-op pass-through. Shipping a half-wired drag with no persistence would violate CLAUDE.md rule 1. Persistence needs either (a) `columnWidths: [CGFloat]?` on `Block.table` (invasive) or (b) side-table on `TableElement` (in-memory only). Dedicated slice.
...
- **7.5.c — UD key proxy + deletion. — PENDING.** Remaining work: make `UserDefaultsManagement.noteFont/.fontName/.fontSize/.codeFont/.codeFontName/.codeFontSize/.editorLineSpacing/.lineHeightMultiple/.lineWidth/.marginSize/.imagesWidth/.italic/.bold` computed getters/setters proxying `Theme.shared`; delete the backing UserDefaults keys (migration already copied them to the active theme on first launch); extend the schema so `lineHeightMultiple`/`italic`/`bold` have flat storage on `BlockStyleTheme` (currently synthesized nested only).
- **7.5.d — Grep gate + doc update. — PENDING.** Run the final Rule 7 / banned-pattern grep across `FSNotes/` and `FSNotesCore/` — zero hits for literal `NSFont.systemFont(ofSize:`, literal `paragraphSpacing = <number>`, literal color in fragment/renderer files (whitelist the UI-layer `previewFontSize` on `PreferencesEditorViewController` if kept literal). Update `ARCHITECTURE.md` to document `Theme` as the single presentation source of truth.
