...
50. Settings → Editor tab is missing 5 checkboxes that the original FSNotes had: Focus in editor when selecting note, Automatically insert closing braces and close quotes, Scan inline tags, Clickable links, Code block live highlighting. User-reported with side-by-side screenshots 2026-04-25. Root cause: commit `7c74195` ("Remove Images Width preference") deleted the Images Width slider's `<slider id="Vrr-sG-fOD">` and its label `dmd-5j-kY7` view elements from `FSNotes/Base.lproj/Main.storyboard` but left 7+1 constraints orphaned referencing those phantom IDs (verified: `rg 'Vrr-sG-fOD' FSNotes/Base.lproj/Main.storyboard` returns 7 constraint hits with zero element-definition hit). The Margin Size label's `top` and `trailing` anchors both reference `Vrr-sG-fOD`; the autolayout chain from Margin Size onward (separator → 5 checkboxes) cascades from an unresolvable anchor. AppKit silently degrades the layout — Margin Size floats, the section separator is misplaced, and the 5 checkboxes get positioned somewhere unrendered. **Fix:** delete the 7+1 phantom constraints; replace the Margin Size top anchor with `3el-ze-TIn.top = bEN-ep-bl6.bottom + 8` (anchored directly to Line Width label) and trailing anchor with whatever the slider chain uses. Outlets and IBActions for all 5 checkboxes are still wired in `PreferencesEditorViewController.swift` — only the storyboard layout chain is broken.
