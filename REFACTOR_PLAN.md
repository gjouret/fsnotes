...
| 12.C.6.h — Link-in-link literalization via §6.4 delimiter stack (specs #518, #519, #520, #532, #533) | ✅ SHIPPED | (this commit) | New `Combinators/LinkResolver.swift` (~250 LoC) carries the §6.4 delimiter-stack algorithm: a pre-pass over the inline character stream that pushes `[` / `![` openers, walks each `]` against the most recent active opener, and — on successful resolution — marks earlier `[` openers (NOT `![`) as inactive. The greedy left-to-right link/image matching previously embedded in `MarkdownParser.tokenizeNonEmphasis` is replaced with a `linksByOpenIdx` lookup populated from the resolver's output. `LinkParser.parseInlineBody` factored out as a public helper so the standalone `LinkParser.match` and the resolver share identical body-parse semantics; `tryMatchReferenceLink` (~70 LoC) retired — its three forms (full / collapsed / shortcut) plus the spec #568 fall-through now live in `LinkResolver.matchAfterCloser`. 16 new tests in `LinkResolverTests`. **Links: 85/90 → 90/90 (100%, closing #518, #519, #520, #532, #533)**. Overall **636/652 (97.5%)**. Two more buckets reach 100%: Links 90/90, and Block quotes 25/25 + Link reference definitions 27/27 already at 100% from earlier 12.C.6 slices. The remaining 16 failures cluster in two families: multi-block list-item children (~107-call-site `ListItem.children: [Block]` redesign, candidate Phase 11/Slice B) and the documented FSNotes++ wikilink-extension non-conformance (#590). |
| 12.C.6 — Push CommonMark from 95.1% to 100% (residual failures) | 🚧 IN PROGRESS | (a–h shipped) | With combinator architecture in place, residuals become small grammar edits. **Slices 12.C.6.a–h shipped: 16 spec fixes against the 620 baseline.** Remaining 16 failures across: Lists + List items (13, multi-block item children — #278, #289, #290, #292, #293, #300, #312, #313, #318, #320, #321, #324, #325 — needs `ListItem.children: [Block]` redesign, ~107 call sites); Tabs (1, #9, tab-as-indent in nested list, tied to multi-block list family); HTML blocks (1, #175, also multi-block list family); Images (1, #590, accepted FSNotes++ wikilink-extension non-conformance). |
...
Eight slices have shipped (12.C.6.a–h, commits `2d05378 → (12.C.6.h)`), closing 16 spec examples (#218, #540, #541, #548, #559, #568, #238, #524, #526, #536, #538, #518, #519, #520, #532, #533) and lifting compliance from 620/652 (95.1%) to 636/652 (97.5%). Three buckets reached 100% (Links 90/90, Block quotes 25/25, Link reference definitions 27/27). See the status-snapshot table at the top of Phase 12 for per-slice details and the matching ARCHITECTURE.md sections for prose write-ups.

The remaining 16 failures cluster into two structural buckets, both larger than the small grammar edits the .a–.h ladder has been delivering:
...
Per-bucket gating remains intact: every commit holds existing buckets at or above their pre-commit pass count, and the threshold floors in `CommonMarkSpecTests` were bumped to lock in each slice's gain (Block quotes 17 → 25, Link reference definitions 22 → 27, Links 75 → 85 → 90).
