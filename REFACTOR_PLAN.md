...
51. Code spans (inline `` `code` ``) render in monospaced font but **without the subtle rounded-rect light-gray chrome** that the original FSNotes drew around them. User-reported with screenshot 2026-04-25. Diagnosis: `theme.chrome.inlineCodeBackground` is defined in `ThemeSchema.swift:297` but **never read** anywhere — `rg -l 'inlineCodeBackground' FSNotesCore/ FSNotes/ -g '*.swift'` returns ONLY the schema definition file. The `.code` inline case in `InlineRenderer.swift:91-96` applies the monospaced font and stops; no `.backgroundColor` attribute, no fragment-level paint pass for code-span ranges. Theme scaffolding that was never wired through. **Two fix levels:** (a) cheap — add `attrs[.backgroundColor] = theme.chrome.inlineCodeBackground.platformColor()` to the `.code` case (~3 LoC, gives solid rect not rounded); (b) full fidelity — paint pass on `ParagraphLayoutFragment` (or a new inline-aware overpaint hook) that traces `.code`-tagged ranges and draws rounded backgrounds, matching the `KbdBoxParagraphLayoutFragment` pattern used for `<kbd>`.
