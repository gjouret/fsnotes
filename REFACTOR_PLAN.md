...
### Status snapshot (2026-04-28, updated post-12.C.6.p)
...
| 12.C.5.g — List multi-line collection port (optional follow-up) | ⏳ OPTIONAL | — | Port the ~320-line block-loop list section + `buildItemTree` (recursive, calls back into `MarkdownParser.parse`) + `deepestOwner` + `stripLeadingSpaces` from `MarkdownParser` into `ListReader`. Closure surface is large (parseInlines, parseRecursive, interruptsLazyContinuation, refDefs); scoped out of 12.C.5.f to keep that slice non-regressing. May not be needed if 12.C.6 lands list-multi-block fixes via small parser edits. |
...
- `FSNotesCore/Rendering/Combinators/` — `Parser.swift` (lib) + 18 ported parsers/resolvers/readers:
  ...
  - **Block readers (12.C.5):** FencedCodeBlockReader, HorizontalRuleReader, ATXHeadingReader, HtmlBlockReader, BlockquoteReader, TableReader, ListReader (per-line classifier only — the multi-line collection logic stays in MarkdownParser pending a potential 12.C.5.g slice).
