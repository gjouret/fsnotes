...
| 12.C.5.d — Blockquote reader port | ✅ SHIPPED | (this commit) | `BlockquoteReader.swift` (~210 LoC). Public `detect(_:)` returns the verbatim `(prefix, content)` pair (covers spec #6 partial-tab consumption). `innerAllowsLazyContinuation(_:)` exposes the FencedCodeBlockReader-aware paragraph-context test. `read(lines:from:trailingNewline:parseInlines:interruptsLazyContinuation:)` walks the explicit-marker + lazy-continuation merge — `parseInlines` and `interruptsLazyContinuation` injected as closures because both depend on parser state (refDef table, list-marker rules) the reader doesn't own. 4 cross-cutting callers rewired (block loop + 2 use-sites of `detectBlockquoteLine` + `interruptsLazyContinuation`). MarkdownParser shrinks by ~120 LoC (37-line block-loop branch collapsed to 11 lines; 2 private helpers — `detectBlockquoteLine`, `blockquoteInnerAllowsLazyContinuation` — moved to the reader). 28 new tests. "Block quotes" 24/25 (96%) held. |
| 12.C.5.e — Table reader port | ⏳ NEXT | — | Multi-line reader with header + separator + body row detection. Already mostly factored as `detectTable`. |
...
- `FSNotesCore/Rendering/Combinators/` — `Parser.swift` (lib) + 15 ported parsers/resolvers/readers:
  ...
  - **Block readers (12.C.5):** FencedCodeBlockReader, HorizontalRuleReader, ATXHeadingReader, HtmlBlockReader, BlockquoteReader. 12.C.5.e+ adds table and list readers.
