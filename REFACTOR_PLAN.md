...
| 12.C.6.b — Unicode case fold for ref label normalization (spec #540) | ✅ SHIPPED | `495c121` | `normalizeLabel` switched from `.lowercased()` to `.folding(options: .caseInsensitive, locale: nil)`. CommonMark §4.7 specifies Unicode case folding (which handles multi-codepoint folds like ẞ → "ss"); `lowercased()` only does single-codepoint case mapping (ẞ → ß), so registration and lookup of labels containing such characters fell out of agreement. Symmetric change — both registration and lookup pass through the same helper. **Links: 76/90 → 78/90 (87%)** (the targeted spec #540 plus one bonus example that fell out of the same fix). Overall CommonMark 621/652 → **623/652 (95.6%)**. |
| 12.C.6 — Push CommonMark from 95.1% to 100% (residual failures) | 🚧 IN PROGRESS | — | With combinator architecture in place, residuals become small grammar edits, not surgery on imperative state. **Slices 12.C.6.a + 12.C.6.b shipped: Link reference definitions 27/27 + Links 78/90 (+3 against the 620 baseline).** Remaining 29 failures across Links (12, delimiter-stack rewrite), Lists + List items (13, multi-block item children), Tabs (1), HTML blocks (1), Block quotes (1), Images (1). |
