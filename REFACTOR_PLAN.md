...
48. Pasting a table copied from an external spreadsheet app (Numbers, Excel, Google Sheets, etc.) does not produce a markdown table. User-reported 2026-04-25. External spreadsheet apps put HTML+RTF on the pasteboard (`public.html`, `public.rtf`) plus tab-separated plain text (`public.utf8-plain-text` with `\t` between cells, `\n` between rows) — none of which is markdown. The current paste path likely treats the plain-text fallback as a single paragraph or falls through to a literal-text dump, so the user sees garbled tabs/newlines instead of a real GFM table. Fix needs a paste-type sniffer at the top of the paste handler: detect TSV (`\t` in any line + ≥2 lines), convert to GFM via a pure helper `tsvToGFMTable(tsv:)`, dispatch the resulting markdown through the existing `EditingOps.replaceBlockRange` / `pasteIntoParagraph` family. HTML-with-`<table>` is a richer fallback path (preserves cell formatting), but TSV is the lowest-bar correct answer. Lives in `EditTextView+Pasting.swift` or wherever the existing paste dispatcher routes pasteboard types.
