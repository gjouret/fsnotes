...
- **4.3 — Non-markdown (`.txt`, `.rtf`) TK2 path. — ✅ SHIPPED 2026-04-23 (commit `94643df`).** New `FSNotesCore/Rendering/NonMarkdownRenderer.swift` with `render(content:bodyFont:theme:)` for `.txt` and `renderRTF(data:bodyFont:theme:)` for `.rtf`. Pure body-font renderer — no markdown parsing, no block model, no marker tagging. Fill path in `EditTextView+NoteState.swift` branches on `!note.isMarkdown()`. `TextStorageProcessor` gains a `nonMarkdownActive` flag (set before fill, reset on next fill) that causes `process()` to bail synchronously, preventing any stray `NotesTextProcessor.highlight*` fire during edits. Three remaining legacy `highlight*` call sites (Preferences, ViewController+Git, EditTextView+Appearance) now gated by `note.isMarkdown()` — stay until 4.4 retires the markdown highlight path entirely. 8 tests in `Phase43NonMarkdownRendererTests`.
