...
Net effect on bypass inventory: **7 → 15 production call sites** across 6 → 7 logical categories. The new category (formatting-IBAction `insertText`) is a latent risk class: any future menu/toolbar IBAction that calls `insertText(_:replacementRange:)` will hit the same trap and must either route through the block model or wrap in `performingLegacyStorageWrite`. Rule 7 gate does not catch this (the grep pattern only flags `performEditingTransaction`); protection is discipline + the Phase 5a runtime assertion during DEBUG dogfood.
...
**5d. Copy / paste — ✅ PARTIAL 2026-04-24 (merged as `0c248f8`; Agent B salvaged from worktree).**
- Shipped (Commits 1–3 of the planned 5-commit sequence):
  ...
  - `FSNotes/View/EditTextView+Clipboard.swift` — copy wire-in routes `markdownForCopy` through `Document.slice` + serializer; `.rtf` branch for cross-app fidelity (TextEdit, Pages, Mail). Paste wire-in adds `insertMarkdownFragmentViaBlockModel` — parses plain-text pasteboard markdown, splices via `EditingOps.insertFragment` for `.paragraph` / `.blankLine` target blocks, falls back to legacy `insertText` for other kinds (list, heading, blockquote, code, table — their kind-aware paste FSMs in `EditingOps.insert(_:at:in:)` remain the right path for those).
  - `Tests/Phase5dCopyPasteSlicingTests.swift` (351 LoC) — round-trip tests for `slice` / `insertFragment` primitives
- Deferred:
  - **Commit 4**: Paste from other apps (`NSAttributedString` on pasteboard, no markdown type) via `InlineRenderer.inlineTreeFromAttributedString` → wrap into `Block.paragraph`s split on hard line breaks. Attribute normalization: keep bold/italic/strike/underline/link; drop font family/size, foreground/background colors, paragraph style, kern, baseline offset. Image pasteboard types (`.png`/`.tiff`) → save to note's `files/` dir → insert as `Inline.image`.
  - **Commit 5**: REFACTOR_PLAN.md ship entry finalization + CLAUDE.md `Current State` refresh.
- **Known issue** to fix in Commit 4: `insertMarkdownFragmentViaBlockModel` calls `handleEditViaBlockModel(in: sel, replacementString: "")` for non-empty selections then `applyEditResultWithUndo(result, "Paste")` — this produces **two undo steps** for paste-over-selection (one to delete, one to insert). Fix: `undoManager?.beginUndoGrouping()` / `endUndoGrouping()` wrap, OR build an `EditingOps.replaceFragment(range:, with:)` primitive that emits one `EditResult`.
- Original scope:
