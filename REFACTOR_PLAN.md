...
### Status snapshot (2026-04-28, updated post-12.C.6.m)
...
Thirteen slices have shipped (12.C.6.a–m, commits `2d05378 → (12.C.6.m)`), closing 27 spec examples (#218, #540, #541, #548, #559, #568, #238, #524, #526, #536, #538, #518, #519, #520, #532, #533, #175, #318, #320, #321, #324, #289, #300, #278, #9, #312, #313) and lifting compliance from 620/652 (95.1%) to 647/652 (99.2%). Five buckets reached 100% (Links 90/90, Block quotes 25/25, Link reference definitions 27/27, HTML blocks 44/44, Tabs 11/11). See the status-snapshot table at the top of Phase 12 for per-slice details and the matching ARCHITECTURE.md sections for prose write-ups.

The remaining 5 failures cluster into two structural buckets, both larger than the small grammar edits the .a–.m ladder has been delivering:

- **Multi-block list-item children (4 failures: #290, #292, #293, #325).** Phase 12.C.6.i closed five (#175, #318, #320, #321, #324) by routing indented block-starters into `continuationBlocks` and re-parsing combined first-line + continuation when the first line opens a non-paragraph block. Phase 12.C.6.j closed #289 (top-level 4-space-indent → indented code) and #300 (setext-underline-as-first-continuation in tight items). Phase 12.C.6.k closed #278 (empty-marker items owning indented code). Phase 12.C.6.l closed #9 (tab expansion in list-item indent). Phase 12.C.6.m closed #312 (lazy continuation of an invalid-marker line into the deepest open paragraph) and #313 (terminate the list when an invalid-marker line follows a blank). The remaining failures need different fixes: 0-space-indented lazy continuation through item content column (#290), nested-blockquote lazy continuation through stripped prefixes (#292, #293), and the `[ListItem]`-vs-`[Block]` ordering issue for paragraph-after-sublist (#325). #325 specifically requires the `ListItem.children: [Block]` redesign so a sublist and a continuation paragraph can interleave correctly.
...
Per-bucket gating remains intact: every commit holds existing buckets at or above their pre-commit pass count, and the threshold floors in `CommonMarkSpecTests` were bumped to lock in each slice's gain (Block quotes 17 → 25, Link reference definitions 22 → 27, Links 75 → 85 → 90, HTML blocks 40 → 44, Lists 9 → 23 → 25, List items 12 → 44 → 45, Tabs 2 → 11).
