...
35. Gutter glyphs (H1/H2/triangle) are not vertically center-justified on their headers. H1's glyph is bottom-aligned; H2-H6 glyphs are top-aligned. Should be vertically centered on the heading's text baseline / cap-height region. Lives in `GutterController.swift` glyph layout.
36. Drag-and-drop on a row/column handle does not draw the blue insertion line during drag and doesn't move the row/column on drop. Regression introduced when handle chrome migrated from fragment-paint to `TableHandleChip` `NSView` subviews (`d391f0b`): `mouseDown` for drag-move was not ported. TK1's `GlassHandleView` had `onDragStart` callback wired through `mouseDown`. Drag-resize for column WIDTH (T2-g.4) is separate code and may still work — bug specifically affects row/column REORDER drag.
