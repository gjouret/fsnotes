...
49. Click in a table cell doesn't place the cursor inside that cell. User-reported 2026-04-25, regression-class. Bug #29 ("Click in top-left cell paints caret ABOVE the cell", `de68ca6`) fixed the caret-paint position but the user reports the click-to-position semantic itself is still broken: clicking inside a cell either does nothing, lands the cursor outside the table, or lands it in the wrong cell. Likely sites: hit-testing in `TableLayoutFragment` (the fragment owns the cell-grid geometry; `cellLocation(forOffset:)` and its inverse `offset(forCellAt:)` cover storage-offset ↔ cell-coordinate, but click-point ↔ cell needs `containsPoint` / `cellAt(localX:localY:)` which is a separate code path). Or `EditTextView`'s `mouseDown` / `selectedRange(forProposedRange:granularity:)` not honouring the table's hit-test result. Diagnose: log the proposed range NSTextView resolves for a click inside a cell; compare to the cell's actual storage-range from `TableElement.cellRange(forCellAt:)`. The mismatch is the bug.
