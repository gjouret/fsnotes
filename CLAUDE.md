...
Phase 5a (commit `7dc480d`) landed the mechanical enforcement: `StorageWriteGuard` (in `FSNotesCore/Rendering/StorageWriteGuard.swift`) exposes four scoped authorization flags (`applyDocumentEditInFlight`, `fillInFlight`, `attachmentHydrationInFlight`, `legacyStorageWriteInFlight`); `TextStorageProcessor.didProcessEditing` traps in `#if DEBUG` when a character-edit happens in WYSIWYG mode (`blockModelActive && !sourceRendererActive`) with no authorization active. Release builds compile to no-ops.

**Bypass-retirement series complete (14 → 0 `legacyStorageWriteInFlight` call sites)**: all original Phase 5a bypasses have been retired by routing through proper primitives, dropped as defensive dead code, or reclassified into dedicated sanctioned-exemption scopes. Verify with `rg -c 'performingLegacyStorageWrite' FSNotes/ FSNotesCore/ -g '*.swift'` (returns 0 in production code; tests still reference the function to verify scope semantics).

The two remaining non-canonical write paths are sanctioned permanent architectural exemptions, not "legacy" code:

1. **IME composition** (Phase 5e) — gated by `compositionAllowsEdit(editedRange:session:)` predicate in `TextStorageProcessor.didProcessEditing`. AppKit's input client writes during marked-text composition; commit folds the result into `Document` via one `applyEditResultWithUndo`.
2. **Async attachment hydration** (display math + mermaid, `EditTextView+BlockModel.swift` ×2) — gated by `StorageWriteGuard.attachmentHydrationInFlight` set via `performingAttachmentHydration { ... }`. The hydrator swaps source text for a rendered NSTextAttachment after `BlockRenderer`'s WebView render completes. Document doesn't change (presentation-only state) so it can't route through `applyDocumentEdit`. Can't be made attribute-only because the source character count differs from the post-hydration U+FFFC count. For mermaid specifically, an attribute-only refactor would actively hurt UX (user wants to see the source diagram syntax during the potentially-slow render, not a blank box of guessed size).

If a new use case appears that needs to write to storage outside `applyDocumentEdit`, the architecturally correct answer is "introduce a dedicated sanctioned scope" (sibling to `attachmentHydrationInFlight`), not "use the legacy escape hatch." The `performingLegacyStorageWrite` function is retained for backwards compatibility but is deprecated for new code.
...
Serializer compliance: **651 / 652 passing (99.8%)** — Phase 10 Slice A baseline (620) advanced by Phase 12.C.6.a–p (+31, sixteen slices closing spec examples #218, #540, #541, #548, #559, #568, #238, #524, #526, #536, #538, #518, #519, #520, #532, #533, #175, #318, #320, #321, #324, #289, #300, #278, #9, #312, #313, #325, #290, #292, #293). Seven buckets reached 100% in 12.C.6: Block quotes (25/25 via #238), Link reference definitions (27/27 via #218), **Links (90/90 via 12.C.6.h)**, **HTML blocks (44/44 via 12.C.6.i)**, **Tabs (11/11 via 12.C.6.l)**, **Lists (26/26 via 12.C.6.n)**, and **List items (48/48 via 12.C.6.o + 12.C.6.p)**.

- Perfect (100%): Precedence, Textual content, Inlines, Code spans, Soft line breaks, Hard line breaks, Blank lines, ATX headings, Setext headings, Backslash escapes, Entity refs, Paragraphs, Fenced code blocks, Autolinks, Indented code blocks, Emphasis, Raw HTML, Thematic breaks, **Tabs** (12.C.6.l), **Link reference definitions** (12.C.6.a), **Block quotes** (12.C.6.f), **Links** (12.C.6.h), **HTML blocks** (12.C.6.i), **Lists** (12.C.6.n), **List items** (12.C.6.o + 12.C.6.p).
- Near-perfect (90%+): Images (21/22, 95%).

Sole remaining failing example:
- **Images (#590)**: wikilink-extension bleed through image ref-def pattern (`![[foo]]` resolves to a wiki link rather than literal text). Accepted FSNotes++ extension non-conformance — wikilinks are a product feature, not a parser bug. 651/652 (99.8%) is the practical ceiling for FSNotes++; pure-CommonMark conformance would land at 652/652 only by removing the wikilink extension.
