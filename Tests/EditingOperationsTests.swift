...
class EditingOperationsTests: XCTestCase {
    ...
    // MARK: - Bug Fix: Empty list item deletion cursor position
    /// When exiting an empty list item that has remaining items after it,
    /// the empty item should just be deleted (no paragraph created) and
    /// cursor placed at end of previous item.
    func test_exitEmptyListItem_withRemainingItems_justDeletes() throws {
        // Create a list where the second item is empty
        let doc = Document(blocks: [
            .list(items: [
                ListItem(indent: "", marker: "-", afterMarker: " ", inline: [.text("one")], children: []),
                ListItem(indent: "", marker: "-", afterMarker: " ", inline: [], children: []),
                ListItem(indent: "", marker: "-", afterMarker: " ", inline: [.text("three")], children: [])
            ])
        ])
        let p = DocumentProjection(document: doc, bodyFont: bodyFont(), codeFont: codeFont())
        
        // Exit the empty second item (inline starts at prefix=1, "one"=3, "\n"=1, prefix=1 = 6)
        let result = try EditingOps.exitListItem(at: 6, in: p)
        let serialized = MarkdownSerializer.serialize(result.newProjection.document)
        
        // Should just have two items, no paragraph
        XCTAssertEqual(serialized, "- one\n- three\n")
        
        // Cursor should be at the end of "one" (prefix=1, "one"=3 = 4)
        XCTAssertEqual(result.newCursorPosition, 4,
                       "Cursor should be at end of previous item (one)")
    }

    /// When exiting the first empty list item, cursor should be at start of list.
    func test_exitEmptyFirstListItem_cursorAtStart() throws {
        // Create a list where the first item is empty
        let doc = Document(blocks: [
            .list(items: [
                ListItem(indent: "", marker: "-", afterMarker: " ", inline: [], children: []),
                ListItem(indent: "", marker: "-", afterMarker: " ", inline: [.text("two")], children: [])
        ...
        // Exit the empty first item (inline starts at prefix=1)
        let result = try EditingOps.exitListItem(at: 1, in: p)
        let serialized = MarkdownSerializer.serialize(result.newProjection.document)
        
        // Should just have one item, no paragraph
        XCTAssertEqual(serialized, "- two\n")
        
        // Cursor should be at start of list (position 0)
        XCTAssertEqual(result.newCursorPosition, 0,
                       "Cursor should be at start of list when first empty item deleted")
    }
}
