...
final class TextKit2FragmentDispatchTests: XCTestCase {
    ...
    // MARK: - Phase 2e — TableBlockAttachment TK2 view provider

    /// Build a `TableBlockAttachment` with a live `InlineTableAttachmentCell`
    /// wrapping a bare `InlineTableView`. This mirrors what
    /// `TableRenderController.renderTables()` does at runtime —
    /// the attachment is emitted by the block-model renderer, then the
    /// app configures a cell on it that holds the live widget.
    private func makeConfiguredTableAttachment(
        bounds: CGRect = CGRect(x: 0, y: 0, width: 400, height: 100)
    ) -> (TableBlockAttachment, InlineTableView) {
        ...
        let cell = InlineTableAttachmentCell(tableView: tableView, size: bounds.size)
        attachment.attachmentCell = cell
        // Mirror what `TableRenderController.renderTables()` does — the
        // TK2 view provider resolves the hosted widget from this weak
        // pointer before falling back to the cell-bridged path.
        attachment.liveHostedView = tableView
        return (attachment, tableView)
    }

    /// TK2 path: `TableBlockAttachment` must vend a
    /// `TableAttachmentViewProvider` via
    /// `viewProvider(for:location:textContainer:)`. Under TK2 the cell's
    /// `draw(withFrame:in:characterIndex:layoutManager:)` is never called,
    /// so the view-provider route is the only way the live
    /// `InlineTableView` reaches the text view's subview tree.
    func test_phase2e_tableAttachment_vendsTableAttachmentViewProvider() {
        let tk2 = makeTK2Container()
        let (attachment, _) = makeConfiguredTableAttachment()
        ...
        XCTAssertNotNil(
            provider,
            "TableBlockAttachment must override viewProvider(...) and " +
            "return a non-nil provider under TK2. Nil under TK2 = falls " +
            "back to the cell's draw() path TK2 never calls, so the " +
            "table is invisible."
        )
        XCTAssertTrue(
            provider is TableAttachmentViewProvider,
            "TableBlockAttachment must vend a TableAttachmentViewProvider " +
            "(got \(provider.map { String(describing: type(of: $0)) } ?? "nil")). " +
            "A generic NSTextAttachmentViewProvider would host an empty view."
        )
    }

    /// TK1 path: under TK1 the container has an `NSLayoutManager`, not
    /// an `NSTextLayoutManager`. The override must return nil so
    /// NSLayoutManager keeps dispatching through
    /// `InlineTableAttachmentCell.draw(...)` — the existing code that
    /// positions the live widget as a text-view subview. Passing
    /// `textContainer: nil` exercises this branch — the override only
    /// consumes the container to reach its layout manager.
    func test_phase2e_tableAttachment_returnsNilProviderUnderTK1() {
        let (attachment, _) = makeConfiguredTableAttachment()
        ...
        XCTAssertNil(
            provider,
            "Under TK1 (textContainer has no NSTextLayoutManager) the " +
            "override must return nil so NSLayoutManager falls back to " +
            "the attachmentCell path. Returning a provider under TK1 " +
            "would double-host the widget (cell.draw() adds it as a " +
            "subview AND the provider would re-add it)."
        )
    }

    /// `loadView` must install the live `InlineTableView` (held by the
    /// `TableAttachmentHosting` cell) as `provider.view` and size its
    /// frame to the attachment's bounds — the same contract the PDF,
    /// image, and bullet view-provider slices pinned. Without this, the
    /// table is clipped, mis-positioned, or shows a default-constructed
    /// `NSView` instead of the actual widget.
    func test_phase2e_tableViewProvider_sizesViewToAttachmentBounds() {
        let tk2 = makeTK2Container()
        let (attachment, tableView) = makeConfiguredTableAttachment(
            bounds: CGRect(x: 0, y: 0, width: 360, height: 140)
        )
        ...
        guard let provider = attachment.viewProvider(
            ...
            XCTFail("Provider must be non-nil under TK2")
            return
        }
        provider.loadView()

        guard let hosted = provider.view else {
            XCTFail("loadView() must populate provider.view")
            return
        }
        XCTAssertTrue(
            hosted === tableView,
            "Provider must host the SAME InlineTableView instance held " +
            "by the attachment's InlineTableAttachmentCell. A fresh view " +
            "would lose focus state, scroll position, and any pending " +
            "field-editor attachments. Got \(type(of: hosted))."
        )
        XCTAssertEqual(
            hosted.frame.width, 360, accuracy: 0.01,
            "Hosted view frame width must equal attachment.bounds.width " +
            "(otherwise the table is clipped or stretched under TK2)."
        )
        XCTAssertEqual(
            hosted.frame.height, 140, accuracy: 0.01,
            "Hosted view frame height must equal attachment.bounds.height."
        )
    }
}
